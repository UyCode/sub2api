export default {
  ...
  admin: {
    ...
    concurrencyTests: {
      title: '并发测试',
      description: '针对上游账号或自定义端点发起并发请求，评估抗压能力、超时和延迟分布',
      searchPlaceholder: '搜索测试名称...',
      createButton: '新增测试',
      createTitle: '新增并发测试',
      editTitle: '编辑并发测试',
      emptyTitle: '暂无并发测试',
      emptyDescription: '创建一个测试配置，用于压测上游账号或自定义端点。',
      loadError: '加载并发测试失败',
      accountsLoadError: '加载账号列表失败',
      saveError: '保存并发测试失败',
      createSuccess: '并发测试已创建',
      updateSuccess: '并发测试已更新',
      deleteSuccess: '并发测试已删除',
      deleteError: '删除并发测试失败',
      deleteConfirm: '确定要删除并发测试「{name}」吗？相关运行结果和日志也会删除。',
      run: '测试',
      running: '测试中',
      runComplete: '并发测试完成',
      runFailed: '并发测试失败',
      ...
      noRunsYet: '还没有运行结果',
      history: '运行历史',
      historyFor: '运行历史：{name}',
      resultTitle: '并发测试结果',
      viewLogs: '查看日志',
      loadRunsError: '加载运行历史失败',
      loadLogsError: '加载运行日志失败',
      customTarget: '自定义端点',
      accountDefaultEndpoint: '按账号平台默认',
      accountCount: '{count} 个账号',
      accountSelection: '账号选择',
      noAccountsLoaded: '未加载到账号',
      endpointPlaceholder: '账号模式可留空；自定义端点必填',
      templateHint: '模板变量：request_index、account_id、timestamp。使用时在请求体里写成双大括号包裹的变量名。OpenAI 编辑图会把 image_base64 转为 multipart 文件；Gemini 编辑图使用 inline_data。',
      invalidJson: 'JSON 解析失败',
      jsonObjectOnly: '必须是 JSON 对象',
      runSummary: '共 {total} 个请求，成功 {success}，失败 {failed}',
      fields: {
        name: '测试名称',
        description: '说明',
        mode: '请求场景',
      ...
      modes: {
        responses: '普通 /v1/responses',
        openaiImageGenerations: 'OpenAI 生图',
        openaiImageEdits: 'OpenAI 编辑图',
        geminiImageGenerations: 'Gemini 生图',
        geminiImageEdits: 'Gemini 编辑图'
      },
      metrics: {
        successRate: '成功率',
        success: '成功',
